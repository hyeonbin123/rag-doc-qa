...
## Goal
FastAPI 공식 문서를 대상으로 한 RAG 문서 QA API. 검색·답변 품질을 측정하면서 개선한다. 진행한 단계:
- 2단계: 평가셋 확장과 검색 개선
- 3단계: Docker 전체 스택 검증
...
- 8단계: 문서를 보기 전에 쓴 새 테스트셋(43문항)에서 답변 정확도를 기준으로 재판정. 동점(4.60)이라 기본값을 dense로 확정. README를 처음 보는 사람이 빨리 파악할 수 있게 재구성하고 실험 기록은 `docs/experiments.md`로 분리

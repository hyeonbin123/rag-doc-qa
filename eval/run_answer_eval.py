...
Usage:
    python -m eval.run_answer_eval [--top-k 5] [--tag v1_baseline] [--skip-judge]
"""
...
async def evaluate_question(
    db, embedder, generator, settings, q: EvalQuestion, top_k: int, skip_judge: bool
) -> dict:
...
async def main(top_k: int, tag: str, skip_judge: bool) -> None:
    ...
    async with async_session_maker() as db:
        for q in questions:
            results.append(
                await evaluate_question(db, embedder, generator, settings, q, top_k, skip_judge)
            )
...
if __name__ == "__main__":
    ...
    asyncio.run(main(args.top_k, args.tag, args.skip_judge))
